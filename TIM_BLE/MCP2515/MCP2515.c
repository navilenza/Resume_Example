...
MCP2515_StatusTypeDef Verify_Oscillator_Presence(void) ;
...
{
  uint8_t dummy;
  HAL_SPI_TransmitReceive(SPI_CAN, &data, &dummy, 1, SPI_TIMEOUT);
}
...
static uint8_t SPI_Rx(void)
{
  uint8_t retVal;
  uint8_t dummy = 0x00;
  HAL_SPI_TransmitReceive(SPI_CAN, &dummy, &retVal, 1, SPI_TIMEOUT);
  return retVal;
}
...
/**
 * @brief Verifies if the oscillator is running by enabling CLKOUT
 * @return MCP2515_OK if the chip responds, MCP2515_ERROR otherwise
 */
MCP2515_StatusTypeDef Verify_Oscillator_Presence(void) {
    MCP2515_Reset();
    HAL_Delay(10);

    MCP2515_WriteByte(MCP2515_CANCTRL, 0x84);
    uint8_t checkValue = MCP2515_ReadByte(MCP2515_CANCTRL);
...
MCP2515_StatusTypeDef MCP2515_Initialize(void)
{
  MCP2515_CS_HIGH();    
...
MCP2515_StatusTypeDef MCP2515_SetConfigMode(void)
{
...
MCP2515_StatusTypeDef MCP2515_SetNormalMode(void)
{
  /* configure CANCTRL Register */
  MCP2515_WriteByte(MCP2515_CANCTRL, 0x08);
...
/* Entering sleep mode */
MCP2515_StatusTypeDef MCP2515_SetSleepMode(void)
{
  /* configure CANCTRL Register */
